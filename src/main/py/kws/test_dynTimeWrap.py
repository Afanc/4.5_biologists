...
class TestDynTimeWrap(TestCase):
    ...
    def test_valid(self):
        features = self.dtw.train(train_pages=range(300, 305), save_file_name=self.paths['valid_features.txt'])
        assert len(features) > 0
    ...
    def test_spot_keywords(self):
        # first train
        self.dtw.train_word_features(self.paths['train_features.txt'])
        # or in memory
        ...
        valid_word_features = self.dtw.load_word_features(self.paths['valid_features.txt'])
        spotted = self.dtw.spot_keywords(valid_word_features, keywords, self.paths['spotting_results.txt'])
