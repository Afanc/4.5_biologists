...
if __name__ == '__main__':
    reportFileName = 'MLP_test_report_out_csv.txt'

    reportFile = open(reportFileName, 'w+')
    reportFile.write('***** MLP parameter test - MNIST data set - 10 epochs per parameter combination***** \n')
        ...
        for learningRate in [1e-3, 3e-3]:
            for dropout in [1, 0.9, 0.8]:
                for hiddenWidth in [512, 1024]:
                    i+=1
                    print("Parameter combination", i, "out of 36")
                    ...
                    os.system(cmd)
                    reportFile = open(reportFileName, 'a')
                    reportFile.write(', ' + str(batchSize) + ', ' + str(
                        learningRate) + ', ' + str(hiddenWidth) + ', ' + str(dropout) + '\n')
                    reportFile.close()